module internal_module
 ...
 ! SR9_v4_NvibAdd: includes Nvib as input argument in gf_method and Wilson. Required for 
 !                 new internal_duschinsky, which allows for reduced dimensionality


 contains
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5

subroutine internal_Wilson(molec,Nvib,S,S_sym,ModeDef,B,G,Asel,verbose,quiet)
    ...
    logical,intent(in),optional :: quiet           ! Suppres all messages
...
!==================================================================================
    ...
    if (present(quiet)) silent_mode=quiet
    ...
    !Initialize matrices
    Nat = molec%natoms
!     Nvib = 3*Nat-6 ! < Now read from input (so as to allow to be lower that vibrational degrees of freedom)
...
subroutine gf_method(Hess,molec,Nvib,S_sym,ModeDef,L,B,G,Freq,Asel,X,Xinv,verbose)
...
subroutine gf_method_V(Hess,molec,Nvib,S_sym,ModeDef,L,B,G,Freq,Asel,X,Xinv,verbose,Grad,Bder)
    ...
    integer,intent(in)          :: Nvib
    real(8),dimension(1:NDIM,1:NDIM),intent(in) ::    G,B    !Wilson matrices
    real(8),dimension(1:NDIM,1:NDIM,1:NDIM),intent(in),optional ::    Bder
    character(len=100),dimension(NDIM),intent(in) :: ModeDef !Definition of modes
    integer,dimension(NDIM),intent(in) :: S_sym              !Manages symmetric internals
    logical,intent(in) :: verbose                            !Verbose level
    !Input-Output
    real(8),dimension(1:NDIM,1:NDIM),intent(inout) :: Hess   !Hessian: cart(in)-intern(out)
    real(8),dimension(1:NDIM),intent(inout),optional :: Grad
    real(8),dimension(NDIM,NDIM),intent(inout) :: Asel       !Manage nonredundant (disabled)
    !Output
    real(8),dimension(NDIM,NDIM),intent(out) :: L,X,Xinv     !Normal modes, G^1/2, G^-1/2 (internal)
    !====================== 
...
subroutine NumBDer(molec,Nvib,S_sym,Bder)
...
! print*, i, "X0+d", molecB%atom(i)%x
        ...
        call internal_Wilson(molecB,Nvib,S,S_sym,ModeDef,Bplus,G,Asel,verbose,quiet=.true.)
        molecB%atom(i)%x = molec%atom(i)%x - delta
! print*, i, "X0-d", molecB%atom(i)%x
        !Call B matrix at this geometry   
        call internal_Wilson(molecB,Nvib,S,S_sym,ModeDef,Bmin,G,Asel,verbose,quiet=.true.)
