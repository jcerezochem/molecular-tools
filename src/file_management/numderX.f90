program fchk2gro
    ...
    open(O_G09,file="L_numder.com")
    molec%title = "L_numder"

    do i=1,molec%natoms
        ! x+
        molec_aux=molec
        write(molec_aux%job%title,'(A,I3,A)') "Atom=", i, " coord: x+"
        molec_aux%atom(i)%x = molec%atom(i)%x + 1e-3
        call write_gcom(O_G09,molec_aux)
        ! x-
        molec_aux=molec
        write(molec_aux%job%title,'(A,I3,A)') "Atom=", i, " coord: x-"
        molec_aux%atom(i)%x = molec%atom(i)%x - 1e-3
        ...
        ! y+
        molec_aux=molec
        write(molec_aux%job%title,'(A,I3,A)') "Atom=", i, " coord: y+"
        molec_aux%atom(i)%y = molec%atom(i)%y + 1e-3
        call write_gcom(O_G09,molec_aux)
        ! y-
        molec_aux=molec
        write(molec_aux%job%title,'(A,I3,A)') "Atom=", i, " coord: y-"
        ...
        ! z+
        molec_aux=molec
        write(molec_aux%job%title,'(A,I3,A)') "Atom=", i, " coord: z+"
        molec_aux%atom(i)%z = molec%atom(i)%z + 1e-3
        call write_gcom(O_G09,molec_aux)
        ! z-
        molec_aux=molec
        write(molec_aux%job%title,'(A,I3,A)') "Atom=", i, " coord: z-"
