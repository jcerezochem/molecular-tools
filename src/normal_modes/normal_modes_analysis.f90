program normal_modes_animation
    ...
    character(len=2) :: scan_type="NM" ! Options: NM, CC, IC
    character(len=2) :: coords="IC"    ! Options: IC, CC
    !Coordinate map
    ...
    ! If internal coordinates, call B first
    if (coords=="IC") call internal_Wilson(molecule,Nvib,S,B,ModeDef)
    !Two possible jobs:
    if (scan_type=="IC" .or. scan_type=="CC") then
        !--------------------------------------
        ! 1. Internal/Cartesian Coordinates SCAN
        !--------------------------------------
        LL(1:Ns,1:Ns)=0.d0
        do i=1,Nsel
            LL(nm(i),nm(i)) = 1.d0
        ...
        if (nmfile == "none") then
            ...
            call gf_method(Nvib,G,Hess,Lint,Freq,X,Xinv)
            if (coords == "IC") then
                ! NM in internal coordinates
                LL=Lint
            else if (vertical) then
                ! NM in Cartesian coordinates with vertical correction
                ! gQ^t * Lint^-1 * Ber
                Lint(1:Nvib,1:Nvib) = inverse_realgen(Nvib,Lint)
                ! gx^t * Lint^-1
!                 do i=i,3*Nat 
            endif
            ! (Otherwise, it is Cartesian coordinates without correction)
    ...
    do jj=1,Nsel 
        ...
        call prepare_files(j,ModeDef(j),scan_type,coords,&
...
!            write(S_VMD,*) "mol addrep 0"
    ...
    subroutine prepare_files(icoord,label,scan_type,coords,&
                             grofile,g09file,g96file,numfile,qfile,title)
        ...
        !Local
        character(len=150) :: dummy_char, label_local, head_title
        character(len=10)  :: suffix, prefix
        ...
        ! Title head 
        head_title="Animation of "//trim(adjustl(prefix))
        ! Label
        if (label == "") then
            label_local=""
        else
            label_local="("//trim(adjustl(label))//")"
        endif
        ! Coord
        write(dummy_char,"(I0,X,A)") icoord

        title   = trim(adjustl(head_title))//" "//trim(adjustl(dummy_char))//trim(label_local)
        g09file = trim(adjustl(prefix))//trim(adjustl(dummy_char))//trim(adjustl(suffix))//".com"
        g96file = trim(adjustl(prefix))//trim(adjustl(dummy_char))//trim(adjustl(suffix))//".g96"
        qfile   = trim(adjustl(prefix))//trim(adjustl(dummy_char))//trim(adjustl(suffix))//"_steps.dat"
        grofile = trim(adjustl(prefix))//trim(adjustl(dummy_char))//trim(adjustl(suffix))//".gro" 
        numfile = trim(adjustl(prefix))//trim(adjustl(dummy_char))//trim(adjustl(suffix))//"_num.com"
