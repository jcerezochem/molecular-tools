program normal_modes_animation
    ...
    ! Description:
    ! -----------
    ! Produce nm animation in terms of either intenal or Cartesian coordinates (in both cases using the Hessian)
    !
...
!   For internal thingies
    ...
    !Other matrices
    real(8),dimension(1:NDIM,1:NDIM) :: Hess, X1,X1inv, L1, Asel1
    !Save definitio of the modes in character
    character(len=100),dimension(NDIM) :: ModeDef
    !VECTORS
    real(8),dimension(NDIM) :: Freq, S1
...
!==================================================================================
    ...
    if (adjustl(coord)=="internal") then
    !====================================
    ...
    else if (adjustl(coord)=="cartesian") then
    !====================================
    !CARTESIAN COORDINATES ANALYSIS
    !====================================
    !We need to
    !1) Rotate to the Eckart frame
    !2) Diagonalize the Hessian 
    !======
    !1) FOLLOWING GAUSSIAN WHITE PAPER
    

    !2) DIAGONALIZE HESSIAN
    call diagonalize_full(Hess(1:3*Nat,1:3*Nat),Nat,L1(1:3*Nat,1:3*Nat),Freq(1:3*Nat),"lapack")
